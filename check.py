...
st.set_page_config(page_title= "Youtube Data Harvesting and Warehousing | By Archana",
                   page_icon= icon,
                   layout= "wide",
                   initial_sidebar_state= "expanded",
                   menu_items={'About': """# This app is created by *Archana!*"""})

# Comfiguring Streamlit GUI 


# Title
st.title(':red[YOUTUBE DATA HARVESTING PROJECT]:computer:')
...
with col1:
    st.header(':orange[Data collection zone]')
    st.write ('(COLLECT DATA FROM :blue [channel id] and **STORED IT IN THE :green[MongoDB] DATABASE**.)')
    channel_id = st.text_input(':green[**Enter channel_id**]')
    st.write('''Get data and stored it in the MongoDB database to click below **:green['Get data and stored']**.''')
    ...
    if Get_data or st.session_state.Get_state:
        ...
        api_version = 'v3'
        api_key = 'AIzaSyClKC_a-EUKaVvdZwGoDwpZZNMRI3c3ekY'
        ...
        client = pymongo.MongoClient("mongodb+srv://archanashrikanth8:mysweety1992@cluster0.9kjhpda.mongodb.net/")
...
with col2:
    ...
    document_name = st.selectbox(':green[**Select Channel name**]', options = document_names, key='document_names')
    ...
    if Migrate or st.session_state_migrate_sql:
        ...
        engine = create_engine('mysql+pymysql://root:PHW#84#jeor@127.0.0.1:3306/youtube_db', echo=False)
...
st.header(':orange[Channel Data Analysis zone]')
...
# -----------------------------------------------------     /   Questions   /    ------------------------------------------------------------- #
st.subheader(':orange[Channels Analysis ]')
...
connect_for_question = pymysql.connect(host='127.0.0.1', user='root', password='PHW#84#jeor',port=3306, db='youtube_db')
